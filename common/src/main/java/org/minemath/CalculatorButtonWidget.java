...
public class CalculatorButtonWidget extends ButtonWidget {
    ...
    protected void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (isClicked && this.isHovered()) {
            context.drawTexture(RenderPipelines.GUI_TEXTURED, textureOnClick, this.getX(), this.getY(), spriteNbr * 16, 0, 16, 13, 560, 13);
        }else if (this.isHovered() && mouseX != midX && mouseY != midY) {
            context.drawTexture(RenderPipelines.GUI_TEXTURED, textureOnHover, this.getX(), this.getY(), spriteNbr * 16, 0, 16, 13, 560, 13);
        } else {
            context.drawTexture(RenderPipelines.GUI_TEXTURED, texture, this.getX(), this.getY(), spriteNbr * 16, 0, 16, 13, 560, 13);
        }
    }
